### part 2 of the day 8 of adventofcode
...
def getPositionOfAntinodes(pair,width,height):
    ...
    # while you are in the grid, keep going and add vectors of iteration of additions or substractions
    while check_point_in_grid(coordinate_first_antinode,width, height):
        antinodes.append(coordinate_first_antinode)
        coordinate_first_antinode = add_vec(coordinate_first_antinode,vector_between_pair)
    while check_point_in_grid(coordinate_second_antinode,width, height):
        antinodes.append(coordinate_second_antinode)
        coordinate_second_antinode = add_vec(coordinate_second_antinode,minus(vector_between_pair))
    return antinodes

# concatenate the list of every antinodes of every pairs
def getAllAntinodes(antennasPairs, width, height):
    antinodes = []
...
def count_unique_locations(allAntinodes):
    unique_locations = {(point[1], point[2]) for point in allAntinodes}
...
if __name__ == "__main__":
    ...
    allAntinodes = getAllAntinodes(antennasPairs,width,height)
    result = count_unique_locations(allAntinodes)
