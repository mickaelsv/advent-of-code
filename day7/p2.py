...
def evaluateExpressions(expression_list):
    ...
    for expression in expression_list:
        ...
        while i+2<n:
            if expression[i+1] == "||":
                outcome_of_current_expression = int(f"{outcome_of_current_expression}{int(expression[i+2])}")

            elif expression[i+1] == "*":
                outcome_of_current_expression*=int(expression[i+2])

            else:
                outcome_of_current_expression+=int(expression[i+2])
            i+=2
        # print("adding", outcome_of_current_expression)
        possible_outcomes.append(outcome_of_current_expression)
...
if __name__ == "__main__":
    ...
    result = 0
    i=0
    for operators in tqdm(everyOperators):
